    ...
    <div className="grid min-h-screen lg:grid-cols-2">
      ...
      <div className="relative flex flex-col bg-slate-50 px-6 py-8 transition-colors duration-200 dark:bg-ink-950 sm:px-12">
        ...
        <div className="relative z-10 mx-auto flex w-full max-w-sm flex-1 flex-col justify-center py-12">
          <div className="mb-8">
            <span className="inline-flex items-center gap-2 rounded-full border border-brand-500/30 bg-brand-500/10 px-3 py-1 text-xs font-medium text-brand-700 dark:text-brand-200">
              <Icon name="lock" className="h-3.5 w-3.5" /> Secure sign in
            </span>
            <h1 className="mt-4 font-display text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Welcome back</h1>
            <p className="mt-1.5 text-sm text-slate-500 dark:text-slate-400">Sign in to continue generating 3D models.</p>
          </div>
...
export function AuthSidePanel({
  ...
  return (
    <div className="relative hidden overflow-hidden border-l border-slate-200/80 bg-slate-100/70 lg:flex lg:flex-col lg:items-center lg:justify-center lg:px-16 dark:border-white/8 dark:bg-ink-900/60">
      ...
      <div className="relative z-10 max-w-md">
        ...
        <h2 className="mt-3 font-display text-3xl font-bold leading-tight tracking-tight text-slate-900 dark:text-white">
          {title}
        </h2>
        <ul className="mt-8 space-y-4">
          {points.map((point) => (
            <li key={point} className="flex items-start gap-3 text-sm text-slate-600 dark:text-slate-300">
              <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-brand-500/15 text-brand-600 dark:text-brand-300">
      ...
      <div className="relative z-10 mt-12 h-56 w-56 animate-float">
        ...
        <span className="absolute inset-4 flex items-center justify-center rounded-3xl border border-slate-200/80 bg-white/80 backdrop-blur-xl dark:border-white/10 dark:bg-ink-950/60">
          <Icon name="cube" className="h-24 w-24 text-brand-500 dark:text-brand-300/70 drop-shadow-[0_0_30px_rgba(124,58,237,0.5)]" strokeWidth={1} />
        </span>
      </div>
    </div>
  );
