...
const fieldClasses = cn(
  ...
  "placeholder:text-slate-400 transition-colors focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20",
  "dark:border-white/12 dark:bg-ink-800/70 dark:text-slate-100 dark:placeholder:text-slate-500 dark:focus:border-brand-400/60 dark:focus:ring-brand-500/25",
);
...
function FieldShell({
  ...
  return (
    <div className="block space-y-1.5">
      ...
      {error && <span className="block text-xs text-red-500 dark:text-red-400">{error}</span>}
    </div>
  );
}
