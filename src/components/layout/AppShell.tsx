...
export function AppShell() {
  ...
  return (
    <div className="flex min-h-screen bg-ink-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
      {/* Ambient background */}
      <div className="pointer-events-none fixed inset-0 z-0 bg-aurora opacity-50 dark:opacity-100" aria-hidden />
      ...
      {sidebarOpen && (
        <div
          className="fixed inset-0 z-30 bg-slate-900/50 backdrop-blur-sm dark:bg-ink-950/70 lg:hidden"
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          ...
          isActive
            ? "bg-brand-50 text-brand-700 shadow-sm dark:bg-brand-500/15 dark:text-brand-200 dark:shadow-[inset_0_1px_0_0_rgba(255,255,255,0.06),0_0_24px_-10px_rgba(139,92,246,0.8)]"
    ...
    >
      {({ isActive }) => (
        <>
          <Icon
            name={item.icon}
            className={cn("h-5 w-5 transition-all", isActive ? "text-brand-600 dark:text-brand-300" : "text-slate-400 group-hover:text-slate-600 dark:text-slate-500 dark:group-hover:text-slate-300")}
            strokeWidth={1.8}
          />
          {item.label}
          {isActive && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-brand-500 shadow-glow" aria-hidden />}
        </>
      )}
    </NavLink>
  );
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <button
        ...
        aria-label="Sign out"
        className="rounded-lg p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-red-500 dark:text-slate-500 dark:hover:bg-white/5 dark:hover:text-red-400"
      >
